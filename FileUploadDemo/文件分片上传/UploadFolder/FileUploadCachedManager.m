...
@interface FileUploadCachedManager()
{
    NSFileManager *_fm;
}

@end
static FileUploadCachedManager *fileManager = nil;
@implementation FileUploadCachedManager
- (instancetype)init
{
    self = [super init];
    if (self) {
...
+ (instancetype)sharedInstance
{
    ...
    dispatch_once(&onceToken, ^{
        if (!self) {
            fileManager  = [[FileUploadCachedManager alloc] init];
        }
    });
    return fileManager;
}
