...
@property (nonatomic, copy) NSString *                          fileSHA1;
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    [aCoder encodeObject:[self fileSHA1] forKey:@"fileSHA1"];
}
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self != nil) {
        ...
        [self setFileSHA1:[aDecoder decodeObjectForKey:@"fileSHA1"]];
    }
...
- (BOOL)getFileInfoAtPath:(NSString*)path {
    ...
    NSString *string = [NSString stringWithSHA256WithData:[NSData dataWithContentsOfFile:path]];
    self.fileData = [NSData dataWithContentsOfFile:path];
    self.fileSHA1 = [NSString stringWithFormat:@"%@.png",string];
    return YES;
}
...
- (void)cutFileForFragments {
    ...
    for (NSUInteger i = 0; i < chunks; i ++) {
        ...
        NSString *SHA256 = [NSString stringWithSHA256WithData:[self readDateOfFragment:fFragment]];
        fFragment.fragmentSHA1 = SHA256;
        fFragment.fragmentData = [self readDateOfFragment:fFragment];
        [fragments addObject:fFragment];
    }
    
    self.fileFragments = fragments;
    for (FileFragment *fFragment in fragments) {
        NSLog(@"-----SHA1--%@---SIZE-%ld",fFragment.fragmentSHA1,fFragment.fragmentSize);
    }
    
    NSMutableArray *array = [[NSMutableArray alloc] init];
    for (FileFragment *file  in self.fileFragments) {
        [array addObject:file.fragmentSHA1];
    }

    self.fileFragmentsSHA256 = array;
}
