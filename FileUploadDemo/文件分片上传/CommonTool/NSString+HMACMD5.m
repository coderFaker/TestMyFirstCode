...
+ (NSString *)stringWithHMACMD5:(NSString *)data key:(NSString *)key {
    ...
    unsigned char keyLen = strlen(cKey);
    CC_MD5_CTX ctxt;
    if (keyLen > blockSize) {
        CC_MD5_Init(&ctxt);
...
+ (NSString *)stringWithSHA1WithData:(NSData *)inputData
{
    ...
    CC_SHA1(inputData.bytes, (unsigned int)inputData.length, digest);
...
//    
    ...
    CC_SHA256(inputData.bytes, (unsigned int)inputData.length, digest);
